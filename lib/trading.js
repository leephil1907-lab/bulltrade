...
const SWAP_FEE = 0.005;        // 0.5% instant-swap fee
...
function equity(userId, mode) {
  ...
  let spot = 0; // held coin balances (instant-swap holdings), valued at live price
  if (w && w.coins) for (const [aid, qty] of Object.entries(w.coins)) {
    const price = markets.priceOf(aid);
    if (price != null && qty > 0) spot += qty * price;
  }
  const allocs = allocationTotals(userId, mode);
  return {
    free: round(free), marginUsed: round(marginUsed), unrealized: round(unrealized),
    spot: round(spot), coins: (w && w.coins) || {},
    inCopy: round(allocs.cash + allocs.marginUsed + allocs.unrealized),
    equity: round(free + spot + marginUsed + unrealized + allocs.cash + allocs.marginUsed + allocs.unrealized)
  };
}
...
function placeOrder(user, o) {
  ...
  const controls = D.db().settings.trading || {};
  if (controls.enabled === false) return { status: 503, error: 'Trading is temporarily disabled by platform controls.' };
  if (mode === 'live' && controls.liveEnabled === false && !user.isBot) return { status: 503, error: 'Live trading is temporarily disabled.' };
  ...
  const maxOrder = Math.max(10, Number(controls.maxOrderUsd) || 1000000);
  if (notional > maxOrder) return { status: 400, error: `Maximum order size is ${maxOrder.toLocaleString()}` };

  const configuredMax = Math.max(1, Number(controls.maxLeverage) || 10);
  const leverage = Math.min(Math.max(parseInt(o.leverage) || 1, 1), asset.maxLeverage || 1, configuredMax);
  ...
  if (type === 'limit') {
    ...
    if (!D.debitWallet(user.id, mode, margin + fee, { type: 'order_reserve', referenceId: 'pending-order', note: `${side} ${asset.symbol} limit` })) return { status: 400, error: 'Insufficient balance to reserve this limit order.' };
    const order = {
      ...
      createdAt: now(), status: 'open', reserved: round(margin + fee)
    };
  ...
  if (!D.debitWallet(user.id, mode, margin + fee, { type: 'trade_open', referenceId: 'pending-position', note: `${side} ${asset.symbol} ${type}` })) return { status: 400, error: 'Insufficient balance. Funds changed before execution; please retry.' };
  const position = {
  ...
  position.ledgerDebitId = 'trade-open:' + position.id;
  D.insert('trades', {
...
function settlePosition(p, exitPrice, reason) {
  ...
  if (p.allocId) {
    ...
    D.creditWallet(p.userId, p.mode, Math.max(returned, 0), { type: liquidated ? 'trade_liquidation_settlement' : 'trade_close', referenceId: p.id, note: `${p.symbol} ${reason}` });
  }
...
function cancelOrder(user, orderId) {
  ...
  const refund = Number(o.reserved || (o.notional / o.leverage + o.fee) || 0);
  if (refund > 0) D.creditWallet(user.id, o.mode, refund, { type: 'order_release', referenceId: o.id, note: 'Limit order cancelled' });
...
function resetDemo(user) {
  ...
  w.coins = {}; // fresh demo = no carried-over coin holdings
  D.save();
...
function startCopy(user, leaderId, mode, amount) {
  ...
  if (!D.debitWallet(user.id, mode, amt, { type: 'copy_allocation', referenceId: leaderId, note: `Copy allocation ${mode}` })) return { status: 400, error: 'Insufficient balance. Please refresh and try again.' };
...
function tick() {
  ...
  for (const o of [...D.db().orders]) {
    ...
    if (!wallet) { D.remove('orders', x => x.id === o.id); continue; }
...
// ---------------- Instant coin swap (spot convert) ----------------
const USD_ASSET = { id: 'usd', symbol: 'USD', name: 'US Dollar', price: 1 };

function swapAssets() {
  return [USD_ASSET].concat(
    markets.snapshot().assets
      .filter(a => a.tradable && a.price != null)
      .map(a => ({ id: a.id, symbol: a.symbol, name: a.name, price: a.price }))
  );
}

function swapQuote(fromId, toId, amount) {
  const all = swapAssets();
  const from = fromId === 'usd' ? USD_ASSET : all.find(a => a.id === fromId);
  const to = toId === 'usd' ? USD_ASSET : all.find(a => a.id === toId);
  if (!from || !to) return { error: 'Unknown asset selected.' };
  if (from.id === to.id) return { error: 'Choose two different coins to swap.' };
  const amt = Number(amount);
  if (!isFinite(amt) || amt <= 0) return { error: 'Enter a valid amount to swap.' };
  const usdValue = amt * from.price;
  if (usdValue < 1) return { error: 'Minimum swap value is $1.' };
  const feeUsd = usdValue * SWAP_FEE;
  const receiveUsd = usdValue - feeUsd;
  return {
    ok: true,
    from: { id: from.id, symbol: from.symbol, price: from.price },
    to: { id: to.id, symbol: to.symbol, price: to.price },
    amount: amt, usdValue: round(usdValue), feeUsd: round(feeUsd),
    receiveUsd: round(receiveUsd), receiveQty: round(receiveUsd / to.price, 8),
    rate: round(from.price / to.price, 8)
  };
}

function executeSwap(user, o) {
  const mode = o.mode === 'live' ? 'live' : 'demo';
  if (mode === 'live') {
    const kyc = D.find('kyc', k => k.userId === user.id);
    if (!kyc || kyc.status !== 'approved') return { status: 403, error: 'KYC verification is required before live swaps. Complete KYC from your dashboard.' };
  }
  const q = swapQuote(o.from, o.to, o.amount);
  if (q.error) return { status: 400, error: q.error };
  const w = D.wallet(user.id, mode, true);
  if (!w.coins) w.coins = {};
  if (o.from === 'usd') {
    if (w.usd + 1e-8 < q.amount) return { status: 400, error: `Insufficient USD balance. Available: $${round(w.usd).toLocaleString()}.` };
    w.usd = round(w.usd - q.amount);
  } else {
    const have = w.coins[o.from] || 0;
    if (have + 1e-8 < q.amount) return { status: 400, error: `Insufficient ${q.from.symbol} balance. Available: ${round(have, 8).toLocaleString()} ${q.from.symbol}.` };
    w.coins[o.from] = round(have - q.amount, 8);
  }
  if (o.to === 'usd') w.usd = round(w.usd + q.receiveQty);
  else w.coins[o.to] = round((w.coins[o.to] || 0) + q.receiveQty, 8);
  D.insert('transactions', {
    id: uid('tx_'), userId: user.id, type: 'swap', mode,
    asset: `${q.from.symbol} → ${q.to.symbol}`,
    fromAsset: q.from.id, toAsset: q.to.id, fromQty: q.amount, toQty: q.receiveQty,
    amountUsd: q.usdValue, feeUsd: q.feeUsd, status: 'completed', createdAt: now()
  });
  gamify.addXP(user.id, 15, 'swap');
  D.save();
  return {
    ok: true,
    message: `Swapped ${q.amount} ${q.from.symbol} → ${q.receiveQty} ${q.to.symbol} (fee $${q.feeUsd}).`,
    quote: q, balances: { usd: w.usd, coins: w.coins }
  };
}
...
module.exports = {
  ...
  SWAP_FEE, swapAssets, swapQuote, executeSwap
};
