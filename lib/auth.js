...
function registerUser(data) {
  ...
  D.insert('wallets', { userId: user.id, mode: 'demo', usd: 0 });
  D.insert('wallets', { userId: user.id, mode: 'live', usd: 0 });
  D.creditWallet(user.id, 'demo', amt, { type: 'signup_bonus', referenceId: user.id, note: 'Initial demo trading balance' });
  // CRM sync is deliberately fire-and-forget: HubSpot must never block account creation.
  if (HubSpot.enabled()) {
    HubSpot.syncContact(user, { lifecyclestage: 'lead' }).catch(() => {});
  }
  return { user };
}
