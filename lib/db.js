...
const { uid, now, hashPassword, round } = require('./utils');
...
const DEFAULTS = () => ({
  ...
  wallets: [],          // hot balance read model; every mutation is journaled in ledger
  ledger: [],            // append-only USD money-movement journal
  ...
  settings: {
    ...
    trading: { enabled: true, liveEnabled: true, maxLeverage: 10, maxOrderUsd: 1000000 }
  }
});
...
function load() {
  try {
    ...
    for (const k of ['contest', 'smtp', 'trading']) {
  ...
  migrateToBots();
  migrateBotsV3();
  // Legacy data may predate the ledger. Seed one immutable opening entry per
  // existing wallet exactly once so the hot wallet read model has a baseline.
  if (state.ledger.length === 0) {
    for (const w of state.wallets) {
      if (Number(w.usd) > 0) {
        Ledger.entry(module.exports, { userId: w.userId, mode: w.mode, direction: 'credit',
          amount: round(w.usd), type: 'opening_balance', referenceId: 'migration-v1', note: 'Opening balance migrated from legacy wallet' });
      }
    }
  }
  save(true);
...
function creditWallet(userId, mode, amount, meta = {}) {
  const value = round(Number(amount));
  if (!isFinite(value) || value <= 0) throw new Error('Credit amount must be positive');
  const w = wallet(userId, mode);
  w.usd = round(w.usd + value);
  Ledger.entry(module.exports, Object.assign({
    userId, mode, direction: 'credit', amount: value,
    type: 'credit', referenceId: '', note: ''
  }, meta));
  save();
  return w;
}

function debitWallet(userId, mode, amount, meta = {}) {
  const value = round(Number(amount));
  if (!isFinite(value) || value <= 0) throw new Error('Debit amount must be positive');
  const w = wallet(userId, mode, false);
  if (!w || w.usd < value) return null;
  w.usd = round(w.usd - value);
  Ledger.entry(module.exports, Object.assign({
    userId, mode, direction: 'debit', amount: value,
    type: 'debit', referenceId: '', note: ''
  }, meta));
  save();
  return w;
}

function ledgerBalance(userId, mode) { return round(Ledger.balance(module.exports, userId, mode)); }
function ledgerEntries(userId, mode, limit) { return Ledger.userEntries(module.exports, userId, mode, limit); }
function reconcileWallet(userId, mode) { return Ledger.reconcile(module.exports, userId, mode); }
...
module.exports = { db, load, save, flush, init, find, filter, insert, update, remove, wallet, creditWallet, debitWallet, ledgerBalance, ledgerEntries, reconcileWallet, logAudit, UPLOAD_DIR, DB_FILE, DEFAULTS };
