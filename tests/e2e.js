...
(async () => {
  ...
  let r = await req('POST', '/api/auth/signup', { name: 'Test Trader', email, password: 'Trader1234!', agree: true, dob: '1995-05-10', country: 'Canada', phone: '+2348012345678', experience: 'some-experience', sourceOfFunds: 'employment', newsletter: true, referredBy: '' });
  ...
  ok('key request blocked without KYC', r.status === 403 && /KYC/.test(r.data.error || ''), r.data.error);
  r = await req('POST', '/api/bots/activate', { botId: bot.id, key: 'BB-WRNG', mode: 'demo', amount: 500 }, userCookie);
  ok('activation blocked without KYC', r.status === 403 && /KYC/.test(r.data.error || ''), r.data.error);
  ...
  ok('deposit locked without KYC (fields validated first)', r.status === 403 && /KYC/.test(r.data.error || ''), r.data.error);
  ...
  const kycDet = { fullName: 'Test Trader', idNumber: 'P1234567', address: '1 Test Lane', city: 'Lagos', country: 'Canada' };
  r = await req('POST', '/api/kyc/submit', { idType: 'passport', details: kycDet, docs: [{ kind: 'front', mime: 'image/png', name: 'id.png', data: png1x1 }, { kind: 'back', mime: 'image/png', name: 'idback.png', data: png1x1 }] }, userCookie);
  ok('kyc submitted (front+back+details) → pending', r.data.ok, r.data);
  r = await req('POST', '/api/kyc/submit', { idType: 'passport', details: kycDet, docs: [{ kind: 'front', mime: 'image/png', name: 'id.png', data: png1x1 }] }, userCookie);
  ok('kyc missing back side rejected', r.status === 400);
  r = await req('POST', '/api/kyc/submit', { idType: 'passport', details: { fullName: '', idNumber: '', address: '' }, docs: [{ kind: 'front', mime: 'image/png', name: 'id.png', data: png1x1 }, { kind: 'back', mime: 'image/png', name: 'idback.png', data: png1x1 }] }, userCookie);
  ok('kyc missing details rejected', r.status === 400);
  ...
  mr = await req('POST', '/api/kyc/submit', { idType: 'passport', details: kycDet, docs: [{ kind: 'front', mime: 'text/html', name: 'x.html', data: 'PGh0bWw+ PGh0bWw+PGh0bWw+' }, { kind: 'back', mime: 'text/html', name: 'y.html', data: 'PGh0bWw+PGh0bWw+' }] }, mr.cookie);
  ...
  // KYC unlocked: bots + deposit now pass the gate (balance rules still apply)
  r = await req('POST', '/api/bots/request-key', { botId: bot.id }, userCookie);
  ok('key request passes KYC, blocked below min balance', r.status === 400 && /minimum/.test(r.data.error || ''), r.data.error);
  r = await req('POST', '/api/bots/activate', { botId: bot.id, key: 'BB-WRNG', mode: 'demo', amount: 500 }, userCookie);
  ok('activation passes KYC, blocked without approved key', r.status === 400 && /key/i.test(r.data.error || ''), r.data.error);
  r = await req('POST', '/api/funding/deposit', { asset: 'BTC', amountUsd: 100, txid: '0x1234567890abcdef1234567890abcdef12345678', proof: { mime: 'image/png', name: 'payment-proof.png', data: PROOF_PNG } }, userCookie);
  ok('deposit accepted after KYC → pending', r.data.ok, r.data);
  // approve deposit
  r = await req('GET', '/api/admin/transactions', null, adminCookie);
  ...
  ok('markets: TradingView chart + coin view present', mktHtml.includes('id="tvWidget"') && mktHtml.includes('coinView') && !mktHtml.includes('bubblesCv'), 'markets');
  ok('markets: pulse strip + TradingView chips', mktHtml.includes('pulseStrip') && mktHtml.includes('id="tvChips"') && mktHtml.includes('data-tv="BTC"'), 'mkt-pulse');
  ok('markets: candle chart lib included', mktHtml.includes('/assets/js/chart.js'));
  const cjs = await (await fetch(BASE + '/assets/js/common.js?v=21')).text();
  ...
  // ========== I18N + FX (language & currency picker) ==========
  {
    ...
    res = await fetch(BASE + '/assets/js/common.js?v=21');
  ...
  // ========== SUPER ADMIN: P/L adjust, user actions, user email ==========
  {
    ...
    r = await req('POST', '/api/auth/signup', { name: 'Super Target', email: suEmail, password: 'Trader1234!', agree: true });
    const suId = r.data.user.id;
    const sc = r.cookie;
    ok('superadmin: target user created', r.data.ok);
    r = await req('POST', '/api/admin/pnl', { userId: suId, mode: 'live', amount: 250, note: 'e2e test' }, adminCookie);
    ok('superadmin: manual P/L +250 applied', r.data.ok && r.data.manualPnl === 250, r.data);
    r = await req('POST', '/api/admin/pnl', { userId: suId, mode: 'live', amount: -70, note: 'e2e test 2' }, adminCookie);
    ok('superadmin: manual P/L -70 applied (→180)', r.data.ok && r.data.manualPnl === 180, r.data);
    r = await req('GET', '/api/auth/me', null, sc);
    ok('superadmin: manualPnl visible to user (180)', r.data.live && r.data.live.manualPnl === 180, r.data.live && r.data.live.manualPnl);
    r = await req('GET', '/api/admin/user?id=' + suId, null, adminCookie);
    ok('superadmin: admin sees manualPnl + kyc details field', r.data.ok && r.data.live.manualPnl === 180 && (r.data.kyc === null || 'details' in r.data.kyc), r.data.live && r.data.live.manualPnl);
    r = await req('POST', '/api/admin/user-email', { userId: suId, subject: 'e2e check', message: 'hello' }, adminCookie);
    ok('superadmin: user email endpoint (suppressed for test domain)', r.data.ok, r.data);
    r = await req('POST', '/api/admin/user-action', { userId: suId, action: 'ban' }, adminCookie);
    ok('superadmin: suspend user', r.data.ok);
    r = await req('POST', '/api/auth/login', { email: suEmail, password: 'Trader1234!' });
    ok('superadmin: suspended user cannot log in', r.status === 403);
    r = await req('POST', '/api/admin/user-action', { userId: suId, action: 'unban' }, adminCookie);
    ok('superadmin: unsuspend user', r.data.ok);
    r = await req('POST', '/api/admin/user-action', { userId: suId, action: 'forcelogout' }, adminCookie);
    ok('superadmin: force logout', r.data.ok);
    r = await req('GET', '/api/auth/me', null, sc);
    ok('superadmin: session invalidated after force logout', r.data.user === null);
    r = await req('POST', '/api/admin/user-action', { userId: suId, action: 'delete', confirm: 'wrong@email.com' }, adminCookie);
    ok('superadmin: delete requires exact email confirm', r.status === 400);
    r = await req('POST', '/api/admin/user-action', { userId: suId, action: 'delete', confirm: suEmail }, adminCookie);
