...
- **Country is the master control (v16):** the globe menu now leads with a searchable, flag-matched **Country / Region** list (all 196 countries; `CTRY` table with ISO codes, flags derived as regional-indicator emoji). Picking a country sets the display currency via `COUNTRY_CUR` (ISO→one of the 19 supported; Eurozone + CFA-franc + Maghreb → EUR, EAC → KES, CFA → EUR, Gulf → own/USD, fallback USD) — respected unless a manual currency pick exists. The signup wizard's country dropdown now shows flags and switches the currency live as you pick; after login, the country saved on the account applies automatically (once). The header button shows the selected country's flag. Swap modal fee/value on /trade now render in the display currency (`BB.fmtUSD`).
